...
<script type="text/javascript" src="${ctx }/js/workflow/trace-process.js"></script>
</head>
<body>${activeActivities}
    <%-- 先读取图片再通过Javascript定位 --%>
    <div>
        <img id="processDiagram" src="${ctx }/workflow/read-resource?pdid=${historicProcessInstance.processDefinitionId}&resourceName=${processDefinition.diagramResourceName}" />
    </div>
    <hr>
    <fieldset>
        <legend>
            流程综合信息-【${processDefinition.name}】
            <button id="changeToAutoDiagram" class="btn btn-info">坐标错位请点击这里</button>
        </legend>
        <table id="pinfo" class="table table-bordered table-hover table-condensed">
            <tr>
                ...
                <td>
                    ...
                    <c:if test="${not empty parentProcessInstance.id}">
                        <a href="${parentProcessInstance.id}" style="margin-left: 2em;">父流程：${parentProcessInstance.id}</a>
                    </c:if>
                </td>
            ...
            <tr>
                ...
                <td>
                    <fmt:formatDate value="${historicProcessInstance.startTime}" pattern="yyyy-MM-dd hh:mm:ss" />
                </td>
                <th width="100">流程结束时间</th>
                <td>
                    <fmt:formatDate value="${historicProcessInstance.endTime}" pattern="yyyy-MM-dd hh:mm:ss" />
                </td>
            ...
            <tbody>
                <c:forEach items="${formProperties}" var="prop">
                    <tr>
                        ...
                        <td>${prop.taskId}</td>
                        <td>
                            <fmt:formatDate value="${prop.time}" pattern="yyyy-MM-dd hh:mm:ss" />
                        </td>
                    </tr>
                </c:forEach>
            </tbody>
